...
const DetailRow = ({ label, value }: { label: string; value: string | undefined }) => (
  <Box sx={{ display: 'flex', flexDirection: 'row', alignItems: 'center', borderBottom: '1px solid rgba(0,0,0,0.05)', py: 1 }}>
    <Typography variant="subtitle1" fontWeight="bold" color="primary.main" sx={{ minWidth: '120px' }}>{label}:</Typography>
    <Typography variant="body1" color="text.primary" sx={{ fontWeight: 500 }}>{value}</Typography>
  </Box>
);
...
const PrayerDetails: React.FC = () => {
  ...
  const handleShareWithWaze
    = () => {
      ...
      let lng = 34.8298554
      window.open(
        `https://waze.com/ul?ll=${lat},${lng}&navigate=yes`, "_blank"
      );
    };
  ...
  const handleShareWithWhatsApp = () => {
    ...
    window.open(
      `https://api.whatsapp.com/send?text=${encodeURIComponent(shareableUrl)}`, "_blank"
    );
  };
